...
fix_display_configurations() {
    ...
    sed -i 's/${env:MONITOR:eDP.*}/${env:MONITOR:'$omw_eDP'}/g' $HOME/.config/polybar/config
    sed -i 's/${env:MONITOR:HDMI.*}/${env:MONITOR:'$omw_HMDI'}/g' $HOME/.config/polybar/config
}
...
fix_laptop_specific_configurations() {
    ...
    sed -i 's/interface = wl.*/interface = '$wlan_int'/g' $HOME/.config/polybar/config
    sed -i 's/interface = enp.*/interface = '$eth_int'/g' $HOME/.config/polybar/config

    if [ -z $mount2 ]; then
        sed -i 's/.*mount-2.*/# mount-2/g' $HOME/.config/polybar/config
    else
        sed -i 's/.*mount-2.*/mount-2 = '$mount2'/g' $HOME/.config/polybar/config
    fi

    sed -i 's/battery =.*/battery = '$bat_name'/g' $HOME/.config/polybar/config
    sed -i 's/adapter =.*/adapter = '$bat_adap'/g' $HOME/.config/polybar/config
    sed -i 's/full-at =.*/full-at = '$bat_full'/g' $HOME/.config/polybar/config
